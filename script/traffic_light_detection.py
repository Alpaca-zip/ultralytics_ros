...
class TrafficLightDetectionNode:
    def __init__(self):
        ...
        self.timer_image = rospy.Timer(
            rospy.Duration(0.1), self.publish_traffic_light_image
        )
    ...
    def publish_traffic_light_image(self, event):
        if self.debug and self.traffic_light_image is not None:
            image_msg = ros_numpy.msgify(
                Image, self.traffic_light_image, encoding="bgr8"
            )
    ...
    def detection_callback(self, msg):
        ...
        if cropped_image is not None and self.is_vertical(cropped_image):
            red_blue_images, cropped_image_with_bbox = self.extract_red_blue_area(
                cropped_image
            )
    ...
    def extract_red_blue_area(self, image):
        ...
        red_blue_images = [
            image[upper_h_c - s : upper_h_c + s, w_c - s : w_c + s, :],
            image[lower_h_c - s : lower_h_c + s, w_c - s : w_c + s, :],
        ]
        cv2.rectangle(
            image, (w_c - s, upper_h_c - s), (w_c + s, upper_h_c + s), (0, 255, 0), 1
        )
        cv2.rectangle(
            image, (w_c - s, lower_h_c - s), (w_c + s, lower_h_c + s), (0, 0, 255), 1
        )
